...
use crate::protected_metadata_cleanup::ProtectedCreateMonitor;
use crate::protected_metadata_cleanup::cleanup_protected_create_targets;
use crate::protected_metadata_cleanup::cleanup_synthetic_mount_targets;
use crate::protected_metadata_cleanup::register_protected_create_targets;
use crate::protected_metadata_cleanup::register_synthetic_mount_targets;
...
pub(crate) fn run_or_exec_bwrap(bwrap_args: BwrapArgs) -> ! {
    ...
    run_bwrap_with_protected_metadata_runtime(bwrap_args);
}

fn run_bwrap_with_protected_metadata_runtime(bwrap_args: BwrapArgs) -> ! {
    let BwrapArgs {
        args,
        preserved_files,
    ...
    let protected_create_monitor = ProtectedCreateMonitor::start(
        &protected_create_targets,
        report_protected_metadata_violation,
    );
...
fn report_protected_metadata_violation(path: &Path) {
    eprintln!(
...
#[cfg(test)]
#[path = "protected_metadata_runtime_tests.rs"]
mod tests;
